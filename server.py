...
competitions = (
    loadCompetitions()
)  #  chargement des compétitions contienues dans le fichier competitions.json
...
@app.route(
    "/showSummary", methods=["POST"]
)  # route pour afficher le résumé des compétitions et des clubs
def showSummary():
    ...
    ]  # liste des clubs dont l'email correspond à celui soumis dans le formulaire
    if matching_clubs:
...
def book(competition, club):
    ...
    if foundClub and foundCompetition:
        ...
        flash(
            "Nous rencontrons des difficultés pour trouver le club ou la compétition, veuillez réessayer"
        )
        return render_template("welcome.html", club=club, competitions=competitions)
